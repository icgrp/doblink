...
namespace eval ocl_util {
  ...
  proc create_vivado_project {hw_platform_info config_info} {
    ...
    if { [string equal $hw_platform_state "pre_synth"] } {
      ...
      # synth platform
      #   hw_emu flow: create vivado project 
      #   hw flow    : create and configure a hybrid GateLvl project and add synth_bb dcp
      if { $is_hw_emu } {
...
#      set dcp_top [file rootname [file tail $bb_synth_dcp]]
#      set_property top $dcp_top [get_filesets sources_1]
    }
  }
  ...
  proc wait_on_first_run_local_ {impl_runs hw_platform_info config_info clk_info} {
    set steps_log           [dict get $config_info steps_log] 
      ...
      if {[dict exists $runs_status "completed_runs:"]} {
        ...
        if {$completed_runs ne ""} {
          foreach _run $completed_runs {
            ...
            if { [is_best_run $_run $wns_threshold $steps_log] } {
  ...
  proc wait_on_first_run_lsf_ {impl_runs hw_platform_info config_info clk_info} {
    set steps_log           [dict get $config_info steps_log] 
    set wait_on_all_runs    [dict get $config_info wait_on_all_impl_runs] 
    set vivado_output_dir   [dict get $config_info vivado_output_dir] 
    set wns_threshold       [dict get $clk_info worst_negative_slack]
    ...
    if {[llength $impl_runs] > 1} {
      ...
      if {$wait_on_all_runs} {
        ...
        if {$is_lsf} {
          ...
          set best_run [wait_on_first_run_lsf_ $impl_runs $hw_platform_info $config_info $clk_info]
        } else {
          # puts "debug: wait_on_first_run_local_"
          set best_run [wait_on_first_run_local_ $impl_runs $hw_platform_info $config_info $clk_info]
        }
      }
    } else {
  ...
  proc generate_fixed_hw_platform {hw_platform_info config_info {open_design false}} {
    ...
    if { [catch {
      ...
      puts "hw_export: write_hw_platform -fixed $include_bit_option $vpl_output_dir/$fixed_xsa"
      write_hw_platform -fixed {*}$include_bit_option $vpl_output_dir/$fixed_xsa
    } catch_res] } {
      ...
      if {![string equal $ooc_xdc_file ""] && [file exists $ooc_xdc_file]} {
        ...
        if {$xdc_file_obj ne ""} {
          ...
          if { $hw_platform_uses_pr && $pr_flow } {
            add_to_steps_log $steps_log "internal step: move_files \[get_files $xdc_file_obj\] -of_objects \[get_reconfig_modules $reconfig_module\]" [fileName]:[expr [lineNumber [info frame]] + $var]
            move_files [get_files $xdc_file_obj] -of_objects [get_reconfig_modules $reconfig_module]
  ...
  proc create_bitstreams_without_implementation { hw_platform_info config_info clk_info } {
    ...
    if { [string equal $link_output_format "pdi"] } {
      if { $hw_platform_uses_pr } {
        ...
        add_to_steps_log $steps_log "internal step: write_device_image $cell_switch -force $out_partial_pdi" [fileName]:[lineNumber [info frame]]
        write_device_image {*}$cell_switch -force $out_partial_pdi
      } else {
        add_to_steps_log $steps_log "internal step: write_device_image -force $out_full_pdi" [fileName]:[lineNumber [info frame]]
        write_device_image -force $out_full_pdi
      }
    } else {
      if { $hw_platform_uses_pr } {
        set cell_switch "-cell $ocl_inst_path"
        if {$is_hw_export} {
          # generate the full bit
        ...
        add_to_steps_log $steps_log "internal step: write_bitstream $cell_switch -force $out_partial_bit" [fileName]:[lineNumber [info frame]]
        write_bitstream {*}$cell_switch -force $out_partial_bit
      } else {
        add_to_steps_log $steps_log "internal step: write_bitstream -force $out_full_bit" [fileName]:[lineNumber [info frame]]
        write_bitstream -force $out_full_bit
      }
    }

    # hw_export suport - generate fixed platform
    # we can't use launch_runs in diskless project 
    if {$is_hw_export} {
      if { [string equal $link_output_format "pdi"] } {
      ...
      # puts "hw_export: call generate_fixed_hw_platform" 
...
#   }
  ...
  proc run_cmd {cmd2} {
    ...
    uplevel {
      # note: output_dir and other variables are accessible in uplevel
  ...
  # only used for multi-strategies, check if a run meets timing
  proc is_best_run {impl_run wns_threshold steps_log } {
  ...
  proc write_report_timing_and_scale_freq { hw_platform_info config_info clk_info is_physopt_enabled} {
    ...
    if {$enable_multi_strategies || ![string equal $link_output_format "dcp"]} {
      set tcl_hook "${tclhook_prefix}_pre_write_bit_pdi.tcl"
    } elseif { $is_physopt_enabled } {
  ...
  proc update_uuid_rom { uuid cell vivado_output_dir steps_log } {
    # when this tcl hook runs, we know we are in prj/proj.runs/impl_*/, so we need to go 3 levels up
    set vivado_output_dir "../../../$vivado_output_dir"
    set steps_log "../../../$steps_log"

    add_to_steps_log $steps_log "internal step: Updating UUID ROM cell '$cell' with UUID: '$uuid'" [fileName]:[lineNumber [info frame]]

    # validate UUID format
    if {![string is xdigit $uuid]} {
      error2file $vivado_output_dir "UUID should be a 128 bit hex value"
      return
    }
    ...
    if {$uuid_length != 32} {
      error2file $vivado_output_dir "UUID should be 128 bits.  Found [expr 4 * $uuid_length] bits"
      return 1
    }

    # validate cell name
    if {[is_empty [get_cells -quiet $cell]]} {
      error2file $vivado_output_dir "Specified UUID cell does not exist in the design: $cell"
      return 1
    }
...
#      puts "INFO: Setting INIT=000000${uuid_muxed_hex_item} on cell $uuid_rom_sub_cell"
    ...
    puts "INFO: UUID ROM successfully updated"
  };

  proc update_logic_uuid_rom { uuid vivado_output_dir steps_log } {
    set uuid_cell [get_cells -hier -filter {BLP_LOGIC_UUID_ROM == 1}]
    if {![is_empty $uuid_cell]} {
      update_uuid_rom $uuid $uuid_cell $vivado_output_dir $steps_log
    } else {

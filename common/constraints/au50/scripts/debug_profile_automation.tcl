...
namespace eval dpa {
  ...
  set aie_trace_s2mm_name "xilinx.com:hls:aie_trace_s2mm"
  ...
  # Limit to number of monitors we can insert
  # NOTE: as a general rule, timing convergence becomes difficult above this limit
  ...
  proc connect_trace_offload_slave { traceOffloadSlave traceMemoryType traceMemory masterPinName \
                                     traceClock traceReset {traceBandwidth 1000} {traceIndex 0} } {
    ...
    if {[get_bd_cells -quiet $traceOffloadSlave] != {}} {
      ...
      if {[string first "memory_subsystem" $vlnv] >= 0} {
        ...
        if {[string first "axi_noc" $vlnv] >= 0} {
          ...
          set perMasterBandwidth [expr int(floor(double($traceBandwidth) / $numMasters))]
          set connections {}
  ...
  proc verify_aie_trace_gmio {} {
    ...
    set residualBandwidth [expr $noc_mult_factor * ($maxBandwidth - $totalBandwidth)]
  ...
  proc add_aie_trace_infrastructure {dpa_opts} {
    ...
    # NOTE: If packet rate is specified (supported in 2020.1), then use it.
    #       Otherwise, calculate based on decorated bandwidths.
    set requiredBandwidth 0
    set maxBandwidth [get_max_bandwidth]
    set totalBandwidth [expr [get_total_requested_qos true] + [get_total_requested_qos false]]
    set residualBandwidth [expr $noc_mult_factor * ($maxBandwidth - $totalBandwidth)]
    ...
    #} else {
      foreach currStream $aieStreams {
        set currBandwidth [get_property -quiet HDL_ATTRIBUTE.DPA_AIE_TRACE_BANDWIDTH $currStream]
        if {$currBandwidth == {}} {set currBandwidth 0}
        #puts "stream = $currStream, BW = $currBandwidth"
        incr requiredBandwidth $currBandwidth
      }
    #}

    puts "--- DPA: NOC QoS: required = $requiredBandwidth, residual = $residualBandwidth"

    # Make sure we're not over-allocating bandwidth
    if {$requiredBandwidth > $residualBandwidth} {
      send_msg_id "DPA-141" "WARNING" "Reached limits of NOC QoS on device. Reduced total QoS for AIE trace to $residualBandwidth MB/s."
      if {[is_drcv]} {
        ::drcv::create_violation AIE-TRACE-01 -d [expr int($residualBandwidth)]
      }

      set factor [expr double($residualBandwidth) / $requiredBandwidth]
      set requiredBandwidth $residualBandwidth

      # Scale value for each individual stream
      foreach currStream $aieStreams {
        set currBandwidth [get_property -quiet HDL_ATTRIBUTE.DPA_AIE_TRACE_BANDWIDTH $currStream]
        if {$currBandwidth == {}} {set currBandwidth 0}
        set newBandwidth [expr int(floor($factor * $currBandwidth))]
        #puts "Setting new bandwidth for $currStream to $newBandwidth = $factor * $currBandwidth"
        set_property HDL_ATTRIBUTE.DPA_AIE_TRACE_BANDWIDTH $newBandwidth $currStream
      }
    } elseif {[is_drcv]} {
      send_msg_id "DPA-141" "INFO" "AI Engine trace estimated bandwidth of $requiredBandwidth MB/s is below NoC limits on device."
      ::drcv::create_affirmation AIE-TRACE-01 -d [expr int($requiredBandwidth)]
    }
    ...
    # Force one data mover per stream as AXI Stream interconnect has given lots of BW problems 
    ...
    if {$numDataMovers > $numStreams} {
      ...
      send_msg_id "DPA-142" "WARNING" "Limited to $numDataMovers data movers (one per stream) to support an estimated bandwidth of $requiredBandwidth MB/s."
      if {[is_drcv]} {
        ::drcv::create_violation AIE-TRACE-02 -d $numDataMovers -d [expr int($requiredBandwidth)]
      }
    } else {
      send_msg_id "DPA-142" "INFO" "Calculated $numDataMovers data movers to support $numStreams streams with an estimated bandwidth of $requiredBandwidth MB/s."
      if {[is_drcv]} {
        ::drcv::create_affirmation AIE-TRACE-02 -d $numDataMovers -d $numStreams -d [expr int($requiredBandwidth)]
      }
    }
    ...
    for { set m 0 } { $m < $numDataMovers } { incr m } {
      ...
      for { set s 0 } { $s < $numStreams } { incr s } {
        # TODO: distribute stream interconnects based on estimated bandwidths
        set interconIndex [expr int(floor(double($s * $numDataMovers) / $numStreams))]
        if {$interconIndex == $m} {
          incr numSlaves

          # Add to requested QoS
          set currStream [lindex $aieStreams $s]
          set currBandwidth [get_property -quiet HDL_ATTRIBUTE.DPA_AIE_TRACE_BANDWIDTH $currStream]
          if {$currBandwidth == {}} {set currBandwidth 0}
          incr requestedQos $currBandwidth

          # Add guidance result for this trace stream
          send_msg_id "DPA-143" "INFO" "AI Engine trace stream $s has an estimated bandwidth of $currBandwidth MB/s."
          if {[is_drcv]} {
            ::drcv::create_affirmation AIE-TRACE-03 -d $s -d [expr int($currBandwidth)]
          }
        }
      }
      
      # Add guidance result for this data mover
      send_msg_id "DPA-144" "INFO" "AIE trace data mover $m is connected to $numSlaves streams with a write QoS of $requestedQos MB/s."
      if {[is_drcv]} {
        if {$requestedQos > 4000} {
          ::drcv::create_violation AIE-TRACE-04 -d $m -d $numSlaves -d [expr int($requestedQos)]
        } else {
          ::drcv::create_affirmation AIE-TRACE-04 -d $m -d $numSlaves -d [expr int($requestedQos)]
        }
      }
      ...
      set_property -quiet CONFIG.ARB_ON_TLAST      1          $interconObj
      set_property -quiet CONFIG.ARB_ON_MAX_XFERS  0          $interconObj
      set_property -quiet CONFIG.ARB_ON_NUM_CYCLES 0          $interconObj
      # Use True Round-Robin (which splits up arbitration evenly amongst active streams)
      set_property -quiet CONFIG.ARB_ALGORITHM     3          $interconObj
      ...
      connect_trace_offload_slave $traceOffloadSlave $traceMemoryType $traceMemory "${dmaName}/m_axi_gmem" \
          $masterClock $masterReset $requestedQos $m
    ...
    for { set s 0 } { $s < $numStreams } { incr s } {
      ...
      set fifoPropName2 [expr { ($slave >= 10) ? "CONFIG.S${slave}_FIFO_MODE" : "CONFIG.S0${slave}_FIFO_MODE" } ]
      set_property $fifoPropName2 1 $interconObj

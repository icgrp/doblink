...
class BaseSoC(SoCCore):
    def __init__(self, toolchain="vivado", sys_clk_freq=int(100e6), with_ethernet=False, with_sata=False, **kwargs):
        ...
        self.submodules.mm2s = mm2s = LiteDRAMDMAReader(self.sdram.crossbar.get_port(data_width=32, reverse=True), with_csr=True)
        self.add_csr("mm2s")
        ...
        self.submodules.s2mm = s2mm = LiteDRAMDMAWriter(self.sdram.crossbar.get_port(data_width=32, reverse=True), with_csr=True)
        self.add_csr("s2mm")
        ...
        if benchmark == 'rendering':
            ...
            self.submodules.digit_recognition = digit_recognition = DigitRecognition5Page(clk_bft, rst_bft, platform, clock_domain='bft')

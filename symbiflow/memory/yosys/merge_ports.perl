...
if (open(my $fh, '<:encoding(UTF-8)', $filename)) {
  ...
  while ($row) {
	...
	$consume_row=1;
	   ...
	   or ($row =~ /.subckt\s+\$__XILINX_RAMB18_TDP_RW\s+(.*)/))
      {
	my $ram_signals;
	my $ram_type;
	my $ramb18=0;
	...
	if (($nline =~ /.param INIT/)
	    ...
	    or ($nline =~ /.subckt/))
	  {
	    ...
	    $row=$nline;
	    if ($nline =~ /\.subckt/) # if the new line is another subckt process it
	      {
	        $consume_row=-1;
	      }
	    else # otherwise send the raw next line to the output
	      {
		 $consume_row=0;
	      }
	...
	for (my $i=0; $i<$cfg_abits; $i++)
	  {
	    ...
	    if ($port0_prefix ne "")
	      {
		print $outf connect_indexed_port($add_port_modifiers,"ADDRARDADDR",$ri,$sigmap{$p0name});
	      }
	    else
	      {
		print $outf connect_indexed_port($add_port_modifiers,"ADDRARDADDR",$ri,"\$false");
	      }
	      
	    if ($port1_prefix ne "")
	      {
		print $outf connect_indexed_port($add_port_modifiers,"ADDRBWRADDR",$ri,$sigmap{$p1name});
	      }
	    else
	      {
		print $outf connect_indexed_port($add_port_modifiers,"ADDRBWRADDR",$ri,"\$false");
	      }
	  }
        # print Dumper(\%sigmap);
	#data in
	if ($port0_write_prefix ne "")
	  {
	    ...
	    for (my $b=0;$b*9<$cfg_dbits;$b++)
	      {
		for (my $i=0; $i<min($cfg_dbits-$b*9,8); $i++)
		  {
		    my $local_index=$i+$b*9;
		    my $p0name=$port0_write_prefix . "DATA[$local_index]" ;
		    if ($cfg_dbits==1)
		      {
			$p0name=$port0_write_prefix . "DATA" ;
		      }
		    my $dindex=$i+$b*8;
		    print $outf "DIADI[$dindex]=$sigmap{$p0name} ";
		  }
		if ($cfg_dbits>$b*9+8)
		  {
		    my $local_index=8+$b*9;
		    my $p0name=$port0_write_prefix . "DATA[$local_index]" ;
		    print $outf "DIPADIP[$b]=$sigmap{$p0name} ";
		  }
	      }
	  }
	if ($port1_write_prefix ne "")
	  {
		...
		for (my $i=0; $i<min($cfg_dbits-$b*9,8); $i++)
		  {
		    my $local_index=$i+$b*9;
		    my $p1name=$port1_write_prefix . "DATA[$local_index]" ;
		    if ($cfg_dbits==1)
		      {
			$p1name=$port1_write_prefix . "DATA" ;
		      }
		    my $dindex=$i+$b*8;
		    print $outf "DIBDI[$dindex]=$sigmap{$p1name} ";
		  }
		if ($cfg_dbits>$b*9+8)
		  {
		    my $local_index=8+$b*9;
		    my $p1name=$port1_write_prefix . "DATA[$local_index]" ;
		    print $outf "DIPBDIP[$b]=$sigmap{$p1name} ";
		  }
	      }
	  }
	...
	if ($port0_read_prefix ne "")
	  {
		...
		for (my $i=0; $i<min($cfg_dbits-$b*9,8); $i++)
		  {
		    my $local_index=$i+$b*9;
		    my $p0name=$port0_read_prefix . "DATA[$local_index]" ;
		    if ($cfg_dbits==1)
		      {
			$p0name=$port0_read_prefix . "DATA" ;
		      }
		    my $dindex=$i+$b*8;
		    print $outf "DOADO[$dindex]=$sigmap{$p0name} ";
		  }
		if ($cfg_dbits>$b*9+8)
		  {
		    my $local_index=8+$b*9;
		    my $p0name=$port0_read_prefix . "DATA[$local_index]" ;
		    print $outf "DOPADOP[$b]=$sigmap{$p0name} ";
		  }
	      }
	  }
	if ($port1_read_prefix ne "")
	  {
		...
		for (my $i=0; $i<min($cfg_dbits-$b*9,8); $i++)
		  {
		    my $local_index=$i+$b*9;
		    my $p1name=$port1_read_prefix . "DATA[$local_index]" ;
		    if ($cfg_dbits==1)
		      {
			$p1name=$port1_read_prefix . "DATA" ;
		      }
		    my $dindex=$i+$b*8;
		    print $outf "DOBDO[$dindex]=$sigmap{$p1name} ";
		  }
		if ($cfg_dbits>$b*9+8)
		  {
		    my $local_index=8+$b*9;
		    my $p1name=$port1_read_prefix . "DATA[$local_index]" ;
		    print $outf "DOPBDOP[$b]=$sigmap{$p1name} ";
		  }
	      }
	  }
	...
	#  Hopefully HLS is only using the same widths as well...
	if ($cfg_dbits==1)
	  {  print $outf ".param READ_WIDTH_A_1 1\n";}
	else
	  {  print $outf ".param READ_WIDTH_A_1 0\n";}
	if (($cfg_dbits>9) and ($cfg_dbits<=18))
    ...
    if ($consume_row==0) # skip the ones read a port enables
	  ...
	  $row = <$fh>;
	    ...
	    # do nothing and process the new row in $row
    }
    else
      {
	$row = <$fh>;

...
def get_phy_tiles(conn, tile_pkey):
    ...
    for (phy_tile_pkey,) in c.execute(
        ...
        c2.execute(
            "SELECT grid_x, grid_y FROM phy_tile WHERE pkey = ?", (phy_tile_pkey,)
        )
        loc = c2.fetchone()
...
def get_fasm_tile_prefix(conn, g, tile_pkey, site_as_tile_pkey):
    ...
    for loc in phy_locs:
        ...
        if not is_vbrk and not gridinfo.bits:
            print(
                "*** WARNING *** Tile {} appears to be missing bitstream data.".format(
                    tile
                ),
                file=sys.stderr,
            )
    ...
    if site_as_tile_pkey is not None:
        c.execute(
            "SELECT site_pkey FROM site_as_tile WHERE pkey = ?", (site_as_tile_pkey,)
        )
        ...
        c.execute("SELECT name FROM site_type WHERE pkey = ?", (site_type_pkey,))
...
def get_tiles(conn, g, roi, synth_loc_map, synth_tile_map, tile_types):
    ...
    for (
        ...
        fasm_tile_prefix = get_fasm_tile_prefix(conn, g, tile_pkey, site_as_tile_pkey)
...
def main():
    ...
    parser.add_argument("--pin_assignments", required=True, type=argparse.FileType("r"))
...
FROM
    ...
    for direct in directs.values():
        ...
        ET.SubElement(
            ...
            "direct",
            {
                "name": "{}_to_{}_dx_{}_dy_{}".format(
                    direct["from_pin"],
                    direct["to_pin"],
                    direct["x_offset"],
                    direct["y_offset"],
                ),
                "from_pin": add_vpr_tile_prefix(direct["from_pin"]),
                "to_pin": add_vpr_tile_prefix(direct["to_pin"]),

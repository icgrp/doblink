    ...
    with open(filename, "r") as f:
        ...
        for x in f1:
            ...
            if len(y) >= 2:
                port = y[0]
                if len(y) > 2:
                    print("Port: " + port + " has more than one partition pin, using the first one")
                ...
                part_pins[port] = { "port":port, "vpr_loc":vpr_loc, "wire": y[1], "port_type": port_type, "is_clock": is_clock}
        return part_pins
    ...
    with DatabaseCache(args.connection_database, read_only=True) as conn:
        ...
        if(args.partition_pins):
            ...
            acceptable_tile_types = set(
                    filter(lambda x:x.startswith("VBRK") 
                        or x.startswith("INT") or ("CLK" in x),
                            map(lambda t: t['type'],
                                g.tilegrid.values())))
            (x_min, x_max, y_min, y_max) = g.dims()
            def in_grid(ds, x,y):
                return(x-ds >= x_min or x+ds <= x_max or y-ds >= y_min or y+ds <= y_max)
            ...
            for pin in partition_pins:
                ...
                while in_grid(ds, x, y) and best is None:
                    i += 1
                    candidates = filter(lambda p: p in g.loc, set([ (x - ds, y), (x + ds, y), (x, y - ds), (x, y + ds)]))
                    candidates = filter(lambda p: (g.gridinfo_at_loc(p).tile_type in acceptable_tile_types)
                            or (not roi.tile_in_roi(prjxray.grid_types.GridLoc(p[0],p[1]))), candidates) 
                    # filter out filled cells, or maybe change this later to accept io sharing
                    candidates = list(filter(lambda p: p not in filled_state, candidates))
                ...
                synth_tiles['tiles'][tile]['pins'].append(
                    {
                        'roi_name':
                            partition_pins[pin]['port'],
                        'wire':
                            partition_pins[pin]['wire'],
                        'pad':
                            partition_pins[pin]['port'],
                        'port_type':
                            partition_pins[pin]['port_type'],
                        'is_clock':
                            partition_pins[pin]['is_clock'],
                    }
                )

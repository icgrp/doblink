...
class impl(gen_basic):
    ...
    def create_page(self, operator, page_num):
        ...
        self.shell.cp_file(
            "./common/script_src/impl_page.tcl",
            self.pr_dir + "/" + operator + "/impl_" + operator + ".tcl",
        )
        tmp_dict = {
            "set page_num": "set page_num " + str(page_num),
            "set operator": "set operator " + operator,
            "set benchmark": "set benchmark " + self.prflow_params["benchmark_name"],
            "set_property SCOPED_TO_CELLS": "set_property SCOPED_TO_CELLS { level0_i/ulp/ydma_1/inst/page"
            + str(page_num)
            + "_inst } [get_files $page_dcp]",
            "link_design -mode default": "link_design -mode default -reconfig_partitions { level0_i/ulp/ydma_1/inst/page"
            + str(page_num)
            + "_inst } -part $part -top level0_wrapper",
        }
        self.shell.cp_dir(
            "./common/constraints/" + self.prflow_params["board"] + "/*",
            self.pr_dir + "/" + operator,
        )
        self.shell.mkdir(self.pr_dir + "/" + operator + "/output")
        self.shell.replace_lines(
            self.pr_dir + "/" + operator + "/impl_" + operator + ".tcl", tmp_dict
        )
        self.shell.write_lines(
            self.pr_dir + "/" + operator + "/run.sh",
        ...
        if map_target == "riscv":
            ...
            map_target, page_num, input_num, output_num = syn_inst.return_map_target(
                operator
            )
            ...
            if riscv_bit == "empty":
                self.shell.replace_lines(
                    self.pr_dir + "/" + operator + "/run.sh",
                    {
                        "vivado": "vivado -mode batch -source impl_"
                        + operator
                        + ".tcl\ncp ../../F005_bits_"
                        + self.prflow_params["benchmark_name"]
                        + "/"
                        + operator
                        + ".bit ../../F001_overlay/riscv_bit_lib/page"
                        + page_num
                        + "_"
                        + str(inst_mem_size / 2048)
                        + "bram"
                ...
                self.shell.replace_lines(
                    self.pr_dir + "/" + operator + "/run.sh",
                    {
                        "vivado": "echo read_checkpoint: 0 seconds > runLogImpl_"
                        + operator
                        + ".log\nvivado"
                    },
                )
                self.shell.replace_lines(
                    self.pr_dir + "/" + operator + "/run.sh",
                    {
                        "vivado": "echo opt: 0 seconds >> runLogImpl_"
                        + operator
                        + ".log\nvivado"
                    },
                )
                self.shell.replace_lines(
                    self.pr_dir + "/" + operator + "/run.sh",
                    {
                        "vivado": "echo place: 0 seconds >> runLogImpl_"
                        + operator
                        + ".log\nvivado"
                    },
                )
                self.shell.replace_lines(
                    self.pr_dir + "/" + operator + "/run.sh",
                    {
                        "vivado": "echo route: 0 seconds >> runLogImpl_"
                        + operator
                        + ".log\nvivado"
                    },
                )
                self.shell.replace_lines(
                    self.pr_dir + "/" + operator + "/run.sh",
                    {
                        "vivado": "echo bit_gen: 0 seconds >> runLogImpl_"
                        + operator
                        + ".log\nvivado"
                    },
                )
    ...
    def run(self, operator):
        ...
        page_num_exist, page_num = self.pragma.return_pragma(
            "./input_src/"
            + self.prflow_params["benchmark_name"]
            + "/operators/"
            + operator

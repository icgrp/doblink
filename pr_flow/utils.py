...
def load_prflow_params(filename):
  ...
  # parse the specifications for the applications
  root_app = xml.etree.ElementTree.parse(filename).getroot()
  functions = root_app.findall('function')
  links = root_app.findall('link')	
  specs = root_app.findall('spec')
  clock =root_app.findall('clock')

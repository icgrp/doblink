...
class syn(gen_basic):
    ...
    def return_io_num(self, io_pattern, file_list):
        ...
        for line in file_list:
            ...
            if len(num_list) > 0 and int(num_list[0].replace(io_pattern, "")):
                max_num = int(num_list[0].replace(io_pattern, ""))
        return max_num
    ...
    def prepare_HW(self, operator, page_num, input_num, output_num):
        ...
        for name in file_list:
            self.shell.cp_file(
                self.overlay_dir + "/" + name,
                self.syn_dir + "/" + operator + "/" + name,
            )

        # prepare the tcl files for out-of-context compilation
        self.shell.write_lines(
            self.syn_dir + "/" + operator + "/syn_page.tcl",
            self.tcl.return_syn_page_tcl_list(operator, ["./leaf.v"]),
        )

        # prepare the leaf verilog files.
        # Id depends on the IO numbers and operator name
        self.shell.write_lines(
            self.syn_dir + "/" + operator + "/leaf.v",
            self.verilog.return_page_v_list(
                page_num, operator, input_num, output_num, True
            ),
            False,
        )
    ...
    def return_bram18_number(self, size_in, input_num, output_num):
        out = (
            int(size_in) / 2048
            + int(self.prflow_params["input_port_bram_cost"]) * input_num
            + int(self.prflow_params["output_port_bram_cost"]) * output_num
            + 1
        )
        return out
    ...
    def prepare_RISCV(self, operator, page_num, input_num, output_num):
        ...
        debug_exist, debug_port = self.pragma.return_pragma(
            "./input_src/"
        ...
        if map_target == "riscv":
            inst_mem_exist, inst_mem_size = self.pragma.return_pragma(
                "./input_src/"
                + self.prflow_params["benchmark_name"]
                + "/operators/"
                + operator
        ...
        utilization_log_list = self.shell.file_to_list(
            self.overlay_dir + "/utilization" + page_num + ".rpt"
        )
        for line in utilization_log_list:
            if self.shell.have_target_string(line, "|   RAMB18       |"):
                line = line.replace(" ", "")
                line_list = line.split("|")
                bram_limit = line_list[7]

        if int(bram_limit) < self.return_bram18_number(
            inst_mem_size, input_num, output_num
        ):
            log_out = open(
                self.syn_dir + "/" + operator + "/runLog_" + operator + ".log", "w"
            )
        ...
        if int(output_num) > 5:
            ...
            log_out.write(
                "No enough output ports. The maximum output port number is 5!\n"
            )
            log_out.close()
            return
        ...
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/run.sh",
            {
                "RISCV_GNU_TOOLCHAIN_INSTALL_PREFIX=": "RISCV_GNU_TOOLCHAIN_INSTALL_PREFIX="
                + self.prflow_params["riscv_dir"]
            },
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/run.sh",
        ...
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/run.sh",
            {"MEM_SIZE=": "MEM_SIZE=" + str(int(inst_mem_size) / 4)},
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/run.sh",
            {"PAGE_NUM=": "PAGE_NUM=" + str(page_num)},
        )
        ...
        self.shell.add_lines(
            self.syn_dir + "/" + operator + "/main.cpp", "//stream", main_cpp_str_list
        )

        # copy the typedefs.h file into out-of-context synthesis dir
        # replace the headerfile defintions
        self.shell.cp_file(
            "input_src/" + self.prflow_params["benchmark_name"] + "/host/typedefs.h",
            self.syn_dir + "/" + operator,
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/typedefs.h",
            {"<hls_stream.h>": '#include "hls_stream.h"'},
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/typedefs.h",
            {"<ap_fixed.h>": '#include "ap_fixed.h"'},
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/typedefs.h",
            {"#define risc": "#define RISCV"},
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/typedefs.h",
            {"<ap_int.h>": '#include "ap_int.h"'},
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/typedefs.h",
            {"<hls_video.h>": '#include "hls_video.h"'},
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/typedefs.h", {"gmp": ""}
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/typedefs.h",
        ...
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/" + operator + ".cpp",
            {"typedefs.h": '#include "typedefs.h"\n#include "firmware.h"'},
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/firmware.h",
            {"// operator": '#include "' + operator + '.h"'},
        )
        self.shell.replace_lines(
            self.syn_dir + "/" + operator + "/sections.lds",
            {"mem : ORIGIN": "         mem : ORIGIN = 0x00000000, LENGTH = " + LENGTH},
        )
        if debug_exist:
            self.shell.replace_lines(
                self.syn_dir + "/" + operator + "/print.cpp",
                {
                    "#define OUTPORT": "#define OUTPORT (0x10000000+8*"
                    + str(output_num)
                    + ")"
                },
            )
        ...
        self.shell.write_lines(
            self.syn_dir + "/" + operator + "/leaf.v",
            self.verilog.return_page_v_list(
                page_num, operator, input_num, output_num, True, True
            ),
            False,
        )
        if riscv_bit == "empty":
            ...
            self.shell.replace_lines(
                self.syn_dir + "/" + operator + "/src/picorv32_wrapper.v",
                {
                    "parameter IS_TRIPLE": "parameter IS_TRIPLE = "
                    + str(is_triple)
                    + ","
                },
            )
            ...
            self.shell.replace_lines(
                self.syn_dir + "/" + operator + "/src/picorv32_wrapper.v",
                {"parameter ADDR_BITS": "parameter ADDR_BITS = " + str(inst_mem_bits)},
            )
        else:
            self.shell.replace_lines(
                self.syn_dir + "/" + operator + "/run.sh",
                {"vivado": "touch page_netlist.dcp\n"},
            )
    ...
    def return_map_target(self, operator):
        src_list = self.shell.file_to_list(
            "./input_src/"
            + self.prflow_params["benchmark_name"]
            + "/operators/"
            + operator
        ...
        HW_exist, page_num = self.pragma.return_pragma(
            "./input_src/"
            + self.prflow_params["benchmark_name"]
            + "/operators/"
            + operator

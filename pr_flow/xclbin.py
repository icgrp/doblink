...
class xclbin(gen_basic):
  ...
  def create_page(self, operator, page_num):
    self.shell.cp_file('common/script_src/gen_xclbin.sh', self.bit_dir+'/gen_xclbin_'+operator+'.sh')
    ...
    self.shell.replace_lines(self.bit_dir+'/gen_xclbin_'+operator+'.sh', tmp_dict)

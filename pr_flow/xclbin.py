...
class xclbin(gen_basic):
    ...
    def create_page(self, operator, page_num):
        self.shell.cp_file(
            "common/script_src/gen_xclbin.sh",
            self.bit_dir + "/gen_xclbin_" + operator + ".sh",
        )
        tmp_dict = {
            "bitstream=": "bitstream=" + operator + ".bit",
        ...
        self.shell.replace_lines(
            self.bit_dir + "/gen_xclbin_" + operator + ".sh", tmp_dict
        )
        self.shell.cp_file(
            "common/metadata/" + self.prflow_params["board"] + "/ydma.xml",
            self.bit_dir + "/" + operator + ".xml",
        )
        self.shell.cp_file("./common/overlay/dynamic_region.xclbin", self.bit_dir)
        os.system("chmod +x " + self.bit_dir + "/gen_xclbin_" + operator + ".sh")

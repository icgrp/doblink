...
class ip_repo(gen_basic):
    ...
    def create_ip(self, operator):
        ...
        self.shell.write_lines(
            self.mono_bft_dir + "/ip_repo/" + operator + "/ip_page.tcl",
            self.tcl.return_ip_page_tcl_list(operator, page_num, file_list),
        )
        self.shell.write_lines(
            self.mono_bft_dir + "/ip_repo/" + operator + "/run.sh",
        ...
        self.shell.write_lines(
            self.mono_bft_dir + "/ip_repo/" + operator + "/qsub_run.sh",
            self.shell.return_run_sh_list(
                self.prflow_params["Xilinx_dir"], "ip_page.tcl"
            ),
            True,
        ...
        self.shell.write_lines(
            ...
            + str(page_num)
            + ".v",
            self.verilog.return_page_v_list(page_num, operator, input_num, output_num),
            True,
        )

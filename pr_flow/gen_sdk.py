...
class gen_sdk:
    ...
    def modify_parameters(self):
        ...
        xdc_file = open(
            self.sdk_dir + "/src/pblocks_" + self.prflow_params["nl"] + ".xdc", "w"
        )
        xdc_file.close()
    ...
    def project_syn_impl_gen(self):
        ...
        tcl_file_dst.write(
            "file copy -force ./prj/floorplan_static.runs/impl_1/floorplan_static_wrapper.sysdef ./prj/floorplan_static.sdk/floorplan_static_wrapper.hdf\n"
        )
        tcl_file_dst.close()

    def project_syn_gen_gen(self):
        os.system(
            "cp ./input_files/script_src/project_syn_gen_"
    ...
    def sdk_gen(self):
        ...
        mk_file.write(
            "    qsub -N 'sdk_'$file -q "
            + self.prflow_params["qsub_grid"]
            + " -m abe -M $emailAddr -l mem=8G  -cwd ./runme.sh\n"
        )
        mk_file.write("    cd ../../../\n")
        mk_file.write("done\n")
        ...
        mk_file.write("done\n")
        mk_file.write(
            "qsub -N sdk_syn_impl -hold_jid $file_list -q "
            + self.prflow_params["qsub_grid"]
            + " -m abe -M $emailAddr -l mem=8G  -cwd ./qsub_syn_impl.sh\n"
        )
        mk_file.close()
        ...
        mk_file.write('emailAddr="' + self.prflow_params["email"] + '"\n')
        mk_file.write(
            "qsub -N sdk_project_syn_gen -q "
            + self.prflow_params["qsub_grid"]
            + " -m abe -M $emailAddr -l mem=8G  -cwd ./qsub_run.sh\n"
        )
        mk_file.write(
            "qsub -N sdk_sub_syn -hold_jid sdk_project_syn_gen  -q "
            + self.prflow_params["qsub_grid"]
            + " -m abe -M $emailAddr -l mem=8G  -cwd ./qsub_sub_syn.sh\n"
        )
        mk_file.close()

...
class config(gen_basic):
        ...
        for key_a in operator_var_dict:
            ...
            for i_a, var_value_a in enumerate(operator_var_dict[key_a]):
                if var_value_a == "Input_1":
                    tmp_str = (
                        "DMA.Output_1->" + key_a + "." + operator_arg_dict[key_a][i_a]
                    )
                    connection_list.append(tmp_str)
                if var_value_a == "Input_2":
                    tmp_str = (
                        "DMA2.Output_1->" + key_a + "." + operator_arg_dict[key_a][i_a]
                    )
                    connection_list.append(tmp_str)
    ...
    def run(self, operators):
        ...
        self.shell.cp_dir(
            "./input_src/" + self.prflow_params["benchmark_name"] + "/sdk/*",
            self.sdk_dir + "/cpp_src",
        )
        ...
        self.shell.replace_lines(
            ...
            + ".cpp",
            {
                "#define BFT_ADDR_SHIFT": "#define BFT_ADDR_SHIFT "
                + str(16 - int(self.prflow_params["addr_bits"]))
            },
        )

        include_list = self.return_include_list_local(
            operators, self.sdk_dir + "/cpp_src/"
        )
        self.shell.add_lines(
            self.sdk_dir
            + "/cpp_src/config_"
        ...
        self.shell.add_lines(
            ...
            + ".cpp",
            '#include "config.h"',
            ['#include "config_' + self.prflow_params["benchmark_name"] + '.h"'],
        )
        ...
        self.shell.write_lines(
            self.sdk_dir + "/run_project_xsdk.sh",
        ...
        self.shell.write_lines(
            self.sdk_dir + "/main.sh",
            self.shell.return_main_sh_list("run_project_xsdk.sh"),
            True,
        )

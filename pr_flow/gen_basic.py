...
class _shell:
    ...
    def return_slurm_command_str(
        ...
        return (
            ...
            + " --job-name="
            + name
            + " --dependency=$(squeue --noheader --format %i --user=$USER --name "
            + hold_jid
            + "| sed -n -e 'H;${x;s/\\n/,/g;s/^,//;p;}') "
            + shell_file
        )
    ...
    def return_main_sh_list(
        ...
        if back_end == "qsub":
            out_list.append(
                self.return_qsub_command_str(
                    run_file, hold_jid, name, q, email, MEM, node_num
                )
            )
        elif back_end == "slurm":
            out_list.append(
                self.return_slurm_command_str(
                    run_file, hold_jid, name, q, email, MEM, node_num
                )
            )
        else:
    ...
    def return_qsub_scan_sh_list(
        self,
        scan_dir,
        run_shell="qsub_run.sh",
        hold_prefix="hls_",
        name_prefix="mono_bft_",
    ):
        return [
            "#!/bin/bash -e",
            ...
            + " -m abe -M $emailAddr -l mem=8G  -cwd ./"
            + run_shell,
            "    cd -",
            "  fi",
            "done",
...
class _verilog:
    ...
    def return_page_v_list(
        ...
        PAYLOAD_BITS = (
            self.prflow_params["payload_bits"] if PAYLOAD_BITS == None else PAYLOAD_BITS
        )
        PACKET_BITS = (
            self.prflow_params["packet_bits"] if PACKET_BITS == None else PACKET_BITS
        )
        NUM_LEAF_BITS = (
            self.prflow_params["addr_bits"] if NUM_LEAF_BITS == None else NUM_LEAF_BITS
        )
        NUM_PORT_BITS = (
            self.prflow_params["port_bits"] if NUM_PORT_BITS == None else NUM_PORT_BITS
        )
        NUM_ADDR_BITS = (
            self.prflow_params["bram_addr_bits"]
            if NUM_ADDR_BITS == None
            else NUM_ADDR_BITS
        )
        NUM_BRAM_ADDR_BITS = (
            self.prflow_params["bram_addr_bits"]
            if NUM_BRAM_ADDR_BITS == None
            else NUM_BRAM_ADDR_BITS
        )
        FREESPACE_UPDATE_SIZE = (
            self.prflow_params["freespace"]
            if FREESPACE_UPDATE_SIZE == None
            else FREESPACE_UPDATE_SIZE
        )
        ...
        lines_list.append(
            "    input wire [" + str(PACKET_BITS) + "-1 : 0] din_leaf_bft2interface,"
        )
        lines_list.append(
        ...
        if int(input_num) == 0:
            lines_list.append("    assign ack_user2interface_1 = 0;")
        ...
        lines_list.append(
            "        .NUM_IN_PORTS(" + str(self.my_max(1, input_num)) + "),"
        )
        lines_list.append("        .NUM_OUT_PORTS(" + str(output_num) + "),")
        lines_list.append(
            "        .NUM_BRAM_ADDR_BITS(" + str(NUM_BRAM_ADDR_BITS) + "),"
        )
        lines_list.append(
            "        .FREESPACE_UPDATE_SIZE(" + str(FREESPACE_UPDATE_SIZE) + ")"
        )
        ...
        if is_riscv == True:
            ...
            for i in range(1, int(input_num) + 1, 1):
                lines_list.append(
                    "       .din"
                    + str(i)
                    + "(dout_leaf_interface2user_"
                    + str(i)
                    + "),"
                )
                lines_list.append(
                    "       .val_in" + str(i) + "(vld_interface2user_" + str(i) + "),"
                )
                lines_list.append(
                    "       .ready_upward"
                    + str(i)
            ...
            for i in range(1, int(output_num) + 1, 1):
                lines_list.append(
                    "       .dout"
                    + str(i)
                    + "(din_leaf_user2interface_"
                    + str(i)
                    + "),"
                )
                lines_list.append(
                    "       .val_out" + str(i) + "(vld_user2interface_" + str(i) + "),"
                )
                lines_list.append(
                    "       .ready_downward"
                    + str(i)
            ...
            for i in range(int(input_num), 0, -1):
                lines_list.append(
                    "        .Input_"
                    + str(i)
                    + "_V_TDATA(dout_leaf_interface2user_"
                    + str(i)
                    + "),"
                )
                lines_list.append(
            ...
            for i in range(int(output_num), 0, -1):
                lines_list.append(
                    "        .Output_"
                    + str(i)
                    + "_V_TDATA(din_leaf_user2interface_"
                    + str(i)
                    + "),"
                )
                lines_list.append(
...
class _tcl:
    ...
    def return_create_bd_cell_tcl_str(self, obj_name, inst_name, obj_type="ip"):
        return (
            "create_bd_cell -type " + obj_type + " -vlnv " + obj_name + " " + inst_name
        )

    def return_connect_bd_stream_tcl_list(
        self, src_name, dest_name, src_port_name="", dest_port_name=""
    ):
        return [
            "connect_bd_net [get_bd_pins /"
            + src_name
    ...
    def return_syn2bits_tcl_list(
        self, jobs=8, prj_dir="./prj/", prj_name="floorplan_static"
    ):
        ...
        return [
            ...
            + prj_dir
            + prj_name
            + ".sdk/floorplan_static_wrapper.xsa",
            "",
        ]

    def return_syn2dcp_tcl_list(
        self, back_end="slurm", prj_dir="./prj/", prj_name="floorplan_static.xpr"
    ):
        if back_end == "slurm":
            threads_num = 8
        else:
    ...
    def return_ip_page_tcl_list(self, fun_name, fun_num, file_list):
        lines_list = [
            "create_project floorplan_static ./prj -part " + self.prflow_params["part"]
        ]
    ...
    def return_hls_tcl_list(self, fun_name):
        ...
        lines_list.append(
            "set_param general.maxThreads " + self.prflow_params["maxThreads"] + " "
        )
    ...
    def return_hls_prj_list(self, fun_name):
        ...
        # to be implemented
        lines_list.append(
            '<project xmlns="com.autoesl.autopilot.project" name="'
            + fun_name
            + '_prj" top="'
            + fun_name
            + '">'
        ...
        lines_list.append(
            ...
            + "/operators/"
            + fun_name
            + '.cpp " sc="0" tb="false" cflags=""/>'
        )
        lines_list.append(
            '        <file name="../../input_src/'
            + self.prflow_params["benchmark_name"]
            + '/host/typedefs.h" sc="0" tb="false" cflags=""/>'
        )
        lines_list.append("    </files> ")
    ...
    def return_impl_tcl_list(self, fun_name, num, overlay="overlay.dcp", IsNet=False):
        ...
        if IsNet:
            lines_list.append(
                "update_design -cell floorplan_static_i/net"
                + str(num)
                + "/inst -black_box"
            )
            lines_list.append(
                "read_checkpoint -cell floorplan_static_i/net"
                + str(num)
                + "/inst ../../F003_syn_"
                + self.prflow_params["benchmark_name"]
                + "/net"
            ...
            lines_list.append(
                "update_design -cell floorplan_static_i/leaf_empty_"
                + str(num)
                + "/inst -black_box -quiet"
            )
            lines_list.append(
                "read_checkpoint -cell floorplan_static_i/leaf_empty_"
                + str(num)
                + "/inst ../../F003_syn_"
                + self.prflow_params["benchmark_name"]
                + "/"
        ...
        if self.prflow_params["PR_mode"] == "quick":
            lines_list.append(
                "if { [catch {place_design -directive Quick } errmsg] } {"
            )
        else:
        ...
        if self.prflow_params["PR_mode"] == "quick":
            lines_list.append(
                "if { [catch {route_design -directive Quick } errmsg] } {"
            )
        else:
        ...
        if IsNet:
            lines_list.append(
                "write_bitstream  -force  -cell floorplan_static_i/net"
                + str(num)
                + "/inst ../../F005_bits_"
                + self.prflow_params["benchmark_name"]
                + "/net_"
            ...
            lines_list.append(
                "write_bitstream  -force  -cell floorplan_static_i/leaf_empty_"
                + str(num)
                + "/inst ../../F005_bits_"
                + self.prflow_params["benchmark_name"]
                + "/"
        ...
        if IsNet:
            lines_list.append(
                "report_utilization -pblocks " + fun_name + " > utilization.rpt"
            )
        else:
            lines_list.append(
                "report_utilization -pblocks p_" + str(num) + " > utilization.rpt"
            )
    ...
    def return_mk_overlay_tcl_list(self):
        lines_list = []
        lines_list.append(
            "set logFileId [open ./runLog_impl_big_static_"
            + str(self.prflow_params["nl"])
            + '.log "w"]'
        )
        lines_list.append("")
        lines_list.append("#####################")
        ...
        for i in range(int(self.prflow_params["nl"])):
            ...
            if (
                self.prflow_params["page" + str(i)].replace("user_kernel", "")
                != self.prflow_params["page" + str(i)]
            ):
                lines_list.append(
                    "read_checkpoint -cell floorplan_static_i/leaf_empty_"
        ...
        lines_list.append(
            "write_checkpoint -force init_placed_"
            + str(self.prflow_params["nl"])
            + ".dcp"
        )
        lines_list.append("set start_time [clock seconds]")
        ...
        lines_list.append('puts $logFileId "route: $total_seconds seconds"')
        lines_list.append(
            "write_checkpoint -force init_routed_"
            + str(self.prflow_params["nl"])
            + ".dcp"
        )
        lines_list.append("set_param bitstream.enablePR 2341")
        ...
        for i in range(int(self.prflow_params["nl"])):
            if (
                self.prflow_params["page" + str(i)].replace("riscv", "")
                != self.prflow_params["page" + str(i)]
                or self.prflow_params["page" + str(i)].replace("user_kernel", "")
                != self.prflow_params["page" + str(i)]
            ):
                lines_list.append(
                    "update_design -cell floorplan_static_i/leaf_empty_"
    ...
    def return_download_bit_tcl_list(self, operators):
        ...
        lines_list.append(
            "current_hw_device [get_hw_devices " + self.prflow_params["device"] + "]"
        )
        lines_list.append(
            "set_property PROBES.FILE {} [get_hw_devices "
            + self.prflow_params["device"]
            + "]"
        )
        lines_list.append(
        ...
        lines_list.append(
            "set_property PROGRAM.FILE {./main.bit} [get_hw_devices "
            + self.prflow_params["device"]
            + "]"
        )
        lines_list.append(
            "program_hw_devices [get_hw_devices " + self.prflow_params["device"] + "]"
        )
        lines_list.append(
...
if __name__ == "__main__":
    modification_dict = {
        "parameter PAYLOAD_BITS0": "parameter PAYLOAD_BITS0 = a,",
    ...
    lines_list = ["hell", "you", "are", "every", "thing"]

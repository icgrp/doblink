...
class runtime(gen_basic):
  ...
  def run(self, operators):
    ...
    self.shell.cp_file('input_src/'+self.prflow_params['benchmark_name']+'/host/*', self.bit_dir)
    ...
    tmp_dict = {'in1[0].range(31': '      in1[0].range(31,  0) = 0x'+str(hex(len(packet_list)/5*2)).replace('L', '').replace('0x','').zfill(8)+';',
                '#define CONFIG_SIZE': '#define CONFIG_SIZE '+str(len(packet_list)/5*2+2)} 
    self.shell.replace_lines(self.bit_dir+'/host.cpp', tmp_dict) 
    self.shell.add_lines(self.bit_dir+'/host.cpp', '// configure packets', packet_list) 

    self.shell.cp_file('common/script_src/gen_runtime.sh', self.bit_dir)
    tmp_dict = {'source vivado_setup' : 'source '+self.prflow_params['Xilinx_dir'],
                'source xrt_setup'    : 'source '+self.prflow_params['xrt_dir']}
    self.shell.replace_lines(self.bit_dir+'/gen_runtime.sh', tmp_dict)
    self.shell.cp_file(self.bit_dir+'/gen_runtime.sh', self.bit_dir+'/run_app.sh')

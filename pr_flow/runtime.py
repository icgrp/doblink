...
class runtime(gen_basic):
    def __init__(self, prflow_params):
        ...
        self.dest_port_offset = (
            self.payload_bits - self.port_bits - self.addr_bits - self.port_bits
        )
        self.src_page_offset = self.payload_bits - self.port_bits - self.addr_bits
        self.src_port_offset = (
            self.payload_bits - self.port_bits - self.addr_bits - self.port_bits
        )
        self.freespace_offset = (
            self.payload_bits
            - self.port_bits
            - self.addr_bits
            - self.port_bits
    ...
    def return_page_num_dict_local(self, operators):
        ...
        for operator in operator_list:
            HW_exist, target = self.pragma.return_pragma(
                "./input_src/"
    ...
    def return_operator_io_argument_dict_local(self, operators):
        ...
        for operator in operator_list:
            ...
            def_str = ""
            for line in file_list:
                if self.shell.have_target_string(line, "("):
                    def_valid = True
                if def_valid:
    ...
    def return_operator_inst_dict_local(self, operators):
        ...
        operator_var_dict = {}
        file_list = self.shell.file_to_list(
            "./input_src/" + self.prflow_params["benchmark_name"] + "/host/top.cpp"
        )
        for operator in operator_list:
            arguments_list = []
            ...
            inst_str = ""
            for line in file_list:
                if self.shell.have_target_string(line, operator + "("):
                    inst_cnt = inst_cnt + 1
                if inst_cnt == 1:
                    line_str = re.sub("\s+", "", line)
                    line_str = re.sub("\t+", "", line_str)
                ...
                if self.shell.have_target_string(line, ")") and inst_cnt == 1:
                    inst_cnt = 2
                ...
                for key_b in operator_var_dict:
                    for i_b, var_value_b in enumerate(operator_var_dict[key_b]):
                        if var_value_a == var_value_b and key_a != key_b:
                            if self.shell.have_target_string(
                                operator_arg_dict[key_a][i_a], "Input"
                            ):
                                tmp_str = (
                                    key_b
                                    + "."
                                    + operator_arg_dict[key_b][i_b]
                                    + "->"
                                    + key_a
                                    + "."
                                    + operator_arg_dict[key_a][i_a]
                                )
                            else:
                                tmp_str = (
                                    key_a
    ...
    def return_config_packet_list_local(self, page_num_dict, connection_list):
        ...
        for str_value in connection_list:
            ...
            src_port = (
                int(src_output.replace("Output_", ""))
                + int(self.prflow_params["output_port_base"])
                - 1
            )
            dest_page = int(page_num_dict[dest_operator])
            dest_port = (
                int(dest_input.replace("Input_", ""))
                + int(self.prflow_params["input_port_base"])
                - 1
            )
            ...
            src_page_packet = src_page_packet + (
                (2 ** self.bram_addr_bits - 1) << self.freespace_offset
            )
            ...
            packet_list.append(
                ...
                + str(packet_num)
                + "].range(63, 32) = 0x"
                + str(hex(value_high)).replace("L", "").replace("0x", "").zfill(8)
                + "; "
            )
            packet_list.append(
                "      in1["
    ...
    def return_call_list_local(self, operators):
        ...
        for operator in operators.split():
            ...
            instr_size_exist, instr_size = self.pragma.return_pragma(
                "./input_src/"
                + self.prflow_params["benchmark_name"]
                + "/operators/"
                + operator
    ...
    def run(self, operators):
        ...
        self.shell.cp_file(
            "input_src/" + self.prflow_params["benchmark_name"] + "/host/*",
            self.bit_dir,
        )
        ...
        connection_list = self.return_operator_connect_list_local(
            operator_arg_dict, operator_var_dict
        )
        self.print_dict(page_num_dict)
        self.print_list(connection_list)
        packet_list = self.return_config_packet_list_local(
            page_num_dict, connection_list
        )
        self.print_list(packet_list)
        tmp_dict = {
            "in1[0].range(31": "      in1[0].range(31,  0) = 0x"
            + str(hex(len(packet_list) / 5 * 2))
            .replace("L", "")
            .replace("0x", "")
            .zfill(8)
            + ";",
            "#define CONFIG_SIZE": "#define CONFIG_SIZE "
            + str(len(packet_list) / 5 * 2 + 2),
        }
        self.shell.replace_lines(self.bit_dir + "/host.cpp", tmp_dict)
        self.shell.add_lines(
            self.bit_dir + "/host.cpp", "// configure packets", packet_list
        )

        self.shell.cp_file("common/script_src/gen_runtime.sh", self.bit_dir)
        tmp_dict = {
            "source vivado_setup": "source " + self.prflow_params["Xilinx_dir"],
            "source xrt_setup": "source " + self.prflow_params["xrt_dir"],
        }
        self.shell.replace_lines(self.bit_dir + "/gen_runtime.sh", tmp_dict)
        self.shell.cp_file(
            self.bit_dir + "/gen_runtime.sh", self.bit_dir + "/run_app.sh"
        )
        xclbin_list_str = "dynamic_region.xclbin"
        for operator in operators.split():
            xclbin_list_str += " " + operator + ".xclbin"
        self.shell.replace_lines(
            self.bit_dir + "/run_app.sh", {"g++": "./app.exe " + xclbin_list_str}
        )
        os.system("chmod +x " + self.bit_dir + "/run_app.sh")
        os.system("chmod +x " + self.bit_dir + "/gen_runtime.sh")

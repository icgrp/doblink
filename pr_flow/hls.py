...
class hls(gen_basic):
    ...
    def create_page(self, fun_name):
        ...
        self.shell.write_lines(
            self.hls_dir + "/main_" + fun_name + ".sh",
        ...
        self.shell.write_lines(
            self.hls_dir + "/" + fun_name + "_prj/hls.app",
            self.tcl.return_hls_prj_list(fun_name),
        )
        self.shell.write_lines(
            self.hls_dir + "/" + fun_name + "_prj/" + fun_name + "/script.tcl",
            self.tcl.return_hls_tcl_list(fun_name),
        )
        if map_target == "HW":
            # if the map target is Hardware, we need to compile the c code through vivado_hls
            self.shell.write_lines(
                self.hls_dir + "/run_" + fun_name + ".sh",
                self.shell.return_run_hls_sh_list(
            ...
            self.shell.write_lines(
                self.hls_dir + "/run_" + fun_name + ".sh",
                self.shell.return_empty_sh_list(),
                True,
            )
            self.shell.write_lines(
                self.hls_dir + "/runLog" + fun_name + ".log", ["hls: 0 senconds"], False
            )

...
def build_do_blink_designs(cfg):
    ...
    for figure, fig_val in cfg.do_blink.fig.items():
        for num_luts, design_cfg in fig_val.items():
            ...
            leaf_int_src_dir = os.path.join(
                project_dir, f"do_blink/symbiflow_benchmark/leaf_interface"
            )
            ...
            hooks = {
                "pre_run": [
                    ...
                    {
                        "cmd": ["rsync", "-a", leaf_int_src_dir, leaf_int_out_dir],
                        "name": "get verilog files",
                    },
                    ...
                    {
                        "cmd": ["rsync", "-a", "output_fun/top.fasm", os.getcwd()],
                        "name": "get top.fasm",
                    },
                ]
            }
            ...
            try:
                ...
                log.info(e)
                total_times = [
                    float(_design_cfg.timeout)
                    for _figure, _fig_val in cfg.do_blink.fig.items()
                    for _num_luts, _design_cfg in _fig_val.items()
                ]
                log.info(
                    f"Experiment failed! Setting total_time to {np.sum(total_times)} seconds."
                )

...
def build_do_blink_designs(cfg):
  ...
  for figure, fig_val in cfg.do_blink.fig.items():
    for num_luts, design_cfg in fig_val.items():
      source_file_dir = os.path.join(project_dir, "do_blink/symbiflow_benchmark/{}/{}".format(figure, num_luts))
      ...
      files = [{'name' : i.split("/")[-1], 'file_type' : 'verilogSource'} for i in source_files]
      files.extend([{'name' : i.split("/")[-1], 'file_type' : 'user'} for i in user_files])
      ...
      tool_options = {
        ...
        "vpr_options": """
                ...
                """.format(cfg.do_blink.vpr_options.bb_factor,
      ...
      name = "_".join([figure, num_luts, re.sub("\D", "", HydraConfig().get().run.dir)])

      hooks = {'pre_run' : [{'cmd': ['cmake', '-S', src_dir, '-B', build_dir], 'name': 'cmake_gen'},
                            {'cmd': ['cmake', '--build', build_dir, '--target', name+"_bit"], 'name': 'cmake_build'}]}
      edam = {
        "files": files,
        "name": name,

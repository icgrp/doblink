    ...
    for figure, fig_val in cfg.do_blink.fig.items():
        for num_luts, design_cfg in fig_val.items():
            verilog_src_dir = os.path.join(
                project_dir, f"do_blink/symbiflow_benchmark/{figure}/{num_luts}/src"
            )
            verilog_out_dir = os.path.join(
                "/tmp", f"do_blink/symbiflow_benchmark/{figure}/{num_luts}/"
            )
            device_files_dir = os.path.join(
                project_dir,
                f"{cfg.do_blink.rrgraph_install_dir}/{design_cfg.device_family}-{design_cfg.device_name}_test",
            )
            ...
            hooks = {
                "pre_run": [
                    ...
                    {
                        "cmd": ["rsync", "-a", device_files_dir, device_files_out_dir],
                        "name": "get device files",
                    },
                    {
                    ...
                    {
                        "cmd": ["make", "output_fun/top.bit"],
                        "name": "make",
                        "timeout": design_cfg.timeout,
                    },
                ]
            }
            edam = {
            ...
            try:
                ...
                top_synth_log = open(
                    glob.glob(
                        "{}/**/top_synth.json.time".format(curr_build_dir),
                        recursive=True,
                    )[0],
                    "r",
                ).read()
                top_eblif_log = open(
                    glob.glob(
                        "{}/**/out.eblif.time".format(curr_build_dir), recursive=True
                    )[0],
                    "r",
                ).read()
                pack_log = open(
                    glob.glob(
                        "{}/**/out.net.time".format(curr_build_dir), recursive=True
                    )[0],
                    "r",
                ).read()
                io_place_log = open(
                    glob.glob(
                        "{}/**/top_io.place.time".format(curr_build_dir), recursive=True
                    )[0],
                    "r",
                ).read()
                place_log = open(
                    glob.glob(
                        "{}/**/out.place.time".format(curr_build_dir), recursive=True
                    )[0],
                    "r",
                ).read()
                route_log = open(
                    glob.glob(
                        "{}/**/out.route.time".format(curr_build_dir), recursive=True
                    )[0],
                    "r",
                ).read()
                fasm_log = open(
                    glob.glob(
                        "{}/**/top.fasm.time".format(curr_build_dir), recursive=True
                    )[0],
                    "r",
                ).read()
                bitgen_log = open(
                    glob.glob(
                        "{}/**/top.bit.time".format(curr_build_dir), recursive=True
                    )[0],
                    "r",
                ).read()

                time_pattern = pp.Combine(
                    pp.Word(pp.nums) + "." + pp.Word(pp.nums)
                ) + pp.Literal("user")

                top_synth_time = float(
                    time_pattern.searchString(top_synth_log).asList()[0][0]
                )
                ...
                io_place_log_time = float(
                    time_pattern.searchString(io_place_log).asList()[0][0]
                )
                ...
                log.info(f"top_synth_time={top_synth_time} seconds")
                log.info(f"top_eblif_time={top_eblif_time} seconds")
                log.info(f"pack_time_time={pack_time_time} seconds")
                log.info(f"io_place_log_time={io_place_log_time} seconds")
                log.info(f"place_log_time={place_log_time} seconds")
                log.info(f"route_log_time={route_log_time} seconds")
                log.info(f"fasm_log_time={fasm_log_time} seconds")
                log.info(f"bitgen_log_time={bitgen_log_time} seconds")
                ...
                log.info(f"Total compile time={total_time} seconds")
                total_times.append(total_time)
            except RuntimeError as e:
                log.info(e)
                total_times.append(design_cfg.timeout)
                log.info(
                    f"Experiment failed! Setting total_time to {design_cfg.timeout} seconds."
                )

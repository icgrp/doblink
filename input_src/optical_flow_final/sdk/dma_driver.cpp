...
int dma_inst::CheckData(void)
{
	...
	for (i = 0; i < MAX_HEIGHT; i++){
		for (j = 0; j < MAX_WIDTH; j++){
			ap_fixed<32, 13> ap_fix_tmp;
			ap_fix_tmp(31, 0) = RxPacket[(i*MAX_WIDTH+j)*2+4];
			double out_x = ap_fix_tmp.to_float();
			ap_fix_tmp(31, 0) = RxPacket[(i*MAX_WIDTH+j)*2+5];
			double out_y = ap_fix_tmp.to_float();
			...
			double error = out_deg - ref_deg[num_pix];

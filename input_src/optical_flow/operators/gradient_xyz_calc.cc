  ...
  GRAD_XY_OUTER: for(int r=0; r<MAX_HEIGHT+2; r++)
  {
    GRAD_XY_INNER: for(int c=0; c<MAX_WIDTH+2; c++)
    {
      ...
      if (r<MAX_HEIGHT && c<MAX_WIDTH){
    	...
    	ap_uint<64> buf;
    	buf(31,0)= STREAM_READ(Input_1);
    	buf(63,32)= STREAM_READ(Input_1);
	...
	gradient_z.write(grad_z);
      ...
      if(r>=4 && r<MAX_HEIGHT && c>=4 && c<MAX_WIDTH)
      {
		...
		STREAM_WRITE(Output_1, temp1);
		y_grad = y_grad/12;
		temp2.range(31,0) = y_grad.range(31,0);
		STREAM_WRITE(Output_2, temp2);
		temp3 = gradient_z.read();
		STREAM_WRITE(Output_3, temp3);
      }
      else if(r>=2 && c>=2)
      {
    	STREAM_WRITE(Output_1, 0);
    	STREAM_WRITE(Output_2, 0);
		temp3 = gradient_z.read();
		STREAM_WRITE(Output_3, temp3);
      }
    }

...
int cascadeClassifier_1   ( MyPoint pt,
                          int_II II[WINDOW_SIZE][WINDOW_SIZE],
                          int_SII SII[SQ_SIZE][SQ_SIZE]
                        );
...
#pragma HLS INTERFACE ap_hs port=Output_1
	  ...
	  LOAD_i: for( i = 0; i < IMAGE_HEIGHT; i++){
	  ...
	  while ( IMAGE_WIDTH/factor > WINDOW_SIZE && IMAGE_HEIGHT/factor > WINDOW_SIZE )
	  {
	    ...
	    imageScalerL1: for ( i = 0 ; i < IMAGE_HEIGHT ; i++ ){
	      imageScalerL1_1: for (j=0;j < IMAGE_WIDTH ;j++){
	        #pragma HLS pipeline
	        if ( j < w2 && i < h2 )
	      	  Output_1.write(Data[(i*y_ratio)>>16][(j*x_ratio)>>16]);
...
)
{
	...
	//hls::stream<ap_uint<32> > Input_1_scale;
	hls::stream<ap_uint<32> > Input_1_shift;
	int k;
	scale(Input_1, Input_1_shift);

	shift(Input_1_shift, Output_1);
...
#pragma HLS INTERFACE ap_hs port=Input_1
#pragma HLS INTERFACE ap_hs port=Output_1
	static unsigned int data[IMAGE_HEIGHT][IMAGE_WIDTH];
	int i, j, k, l;
	for (i=0; i<IMAGE_HEIGHT; i++){
		for (j=0; j<IMAGE_WIDTH; j++){
			data[i][j]=Input_1.read();
...
#pragma HLS INTERFACE ap_hs port=Output_1
	...
	/** Image Line buffer ( 24 BRAMs ) */
	unsigned char L[42][IMAGE_WIDTH];
	#pragma HLS array_partition variable=L complete dim=1
...
#pragma HLS unroll
	...
	int sum = 0;
	out_1: for (i = 0; i< IMAGE_WIDTH; i++){
#pragma HLS PIPELINE II=1
		sum = 0;
		out_2: for(j = 0; j<42; j++){
#pragma HLS unroll
			sum = sum + L[j][i];
		}
		Output_1.write(sum);

...
int dma_inst::WR2TxBuffer0()
{
	...
	for(i = 0; i < number_of_packets*max_pkt_len/4; i++) {
		TxPacket[i]   = data_in[i];
	}

	for(i = 0; i < number_of_packets_rx*max_pkt_len_rx/4; i++) {
		RxPacket[i]   = 0;
	}

	//printf("TxPacket[%d] = %d\n", number_of_packets*max_pkt_len/4-1, TxPacket[number_of_packets*max_pkt_len/4-1]);
...
int dma_inst::RecvWait()
{
	...
	while (ProcessedBdCount < number_of_packets_rx) {
		...
		ProcessedBdCount += XAxiDma_BdRingFromHw(RxRingPtr,
					       XAXIDMA_ALL_BDS, &BdPtrGlobal);
		//if(total<10) printf("ProcessedBdCount=%x\n", ProcessedBdCount);
		total++;
	}
...
int dma_inst::CheckData(void)
{
	...
	int err_cnt = 0;
	for(i = 0; i < 1; i++) {
		if(RxPacket[i*512+3] != y[i]){
			err_cnt++;
			printf("#%d [Fail]! Pred/Label: %d/%d\n", i, RxPacket[i*512+3], y[i]);
		}else{
			printf("#%d [ OK ]! Pred/Label: %d/%d\n", i, RxPacket[i*512+3], y[i]);
		}
	}
	printf("We got %d / 1 errors!\n", err_cnt);
...
int dma_inst::TxRxSetup(int DoTxSetup, int DoRxSetup){
	int Status;
...
int dma_inst::print_results()
{
	...
	double Tpt_tx = max_pkt_len*number_of_packets/Total_time;
	double Tpt_rx = max_pkt_len_rx*number_of_packets_rx/Total_time;
	printf("Overall Time: %.2lf us\n", Total_time);
	printf("BNN: %.2lf ms / image\n", Total_time/1/1000);
	printf("Tx throughput: %.2lf MB/s\n", Tpt_tx);
	printf("Rx throughput: %.2lf MB/s\n", Tpt_rx);
	return XST_SUCCESS;
}

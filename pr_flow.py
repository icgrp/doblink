...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-q",
        "--run_qsub",
        help="default: don't submit the qsub job to icgrid",
        action="store_true",
    )
    parser.add_argument(
        "-g",
        "--gen_overlay",
        help="default: don't compile the static region",
        action="store_true",
    )
    parser.add_argument(
        "-hls",
    ...
    parser.add_argument(
        "-syn",
        "--gen_syn",
        help="default: don't perform out-of-context synthesis",
        action="store_true",
    )
    parser.add_argument(
        "-impl",
        "--gen_impl",
        help="default: don't perform placement, routing and bitstream generation",
        action="store_true",
    )
    parser.add_argument(
        "-bit",
        "--gen_bits",
        help="default: don't update the download.tcl file for loading bitstreams",
        action="store_true",
    )
    parser.add_argument(
        "-ip",
        "--gen_ip_repo",
        help="default: don't generate ip_repo",
        action="store_true",
    )
    parser.add_argument(
        "-cfg",
        "--gen_config",
        help="generate the configuration packets for PSNoC",
        action="store_true",
    )
    parser.add_argument(
        "-mbft",
        "--gen_mono_bft",
        help="default: don't generate monolithic BFT project",
        action="store_true",
    )
    parser.add_argument(
        "-rpt",
        "--gen_report",
        help="default: don't generate the report",
        action="store_true",
    )
    parser.add_argument(
        "-op",
        ...
        type=str,
        default="no_func",
        help="choose which function to be regenrated",
    )
